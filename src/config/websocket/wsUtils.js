...
if (typeof persistenceDir === 'string') {
  ...
  persistence = {
    bindState: async (identifier, ydoc) => {
      ...
      // let decodedId = decodeURIComponent(identifier);
      let docId = Number(new URLSearchParams(identifier).get('docId'));
      ...
      // console.log(`Comments for document with id: ${document.id}`, document.comments);
      const initialState = document?.y_doc_state;
      if (initialState) {
        let binaryEncoded = toUint8Array(initialState);
        // console.log('applied initial state', decodedId);
 ...
 * @param {any} opts
 */
exports.setupWSConnection = prismaClient => {
  ...
  return (
    ...
    console.log('Starting WS connection for user', req.session.passport.user);
