  ...
  try {
    const createdComment = await prisma.comment.create({
      data: {
        ...
        parentComment: !parentCommentId ? undefined : { connect: { id: parentCommentId} }
      },
      include: {
        usersDocuments: {
          select: { user: true, commentId: true, documentId: true, createdAt: true }
        },
      }
    });
...
router.delete('/:commentId',
  ...
  router.delete('/', requireAuth, async (req, res, next) => {
    ...
    console.log('Could not delete comments for document with id', documentId, error);
    next(error);
  }
})